...
fn main() {
    ...
    let rom = load_rom(&env::args().nth(1).unwrap()).unwrap();

    assert_eq!(MEM_SIZE, rom.len());

    let mut start = str::parse::<MemoryLocation>(&env::args().nth(2).unwrap()).unwrap();
    let num = env::args().nth(3)
        .and_then(|v| str::parse::<u16>(&v).ok())
        .unwrap_or_else(|| 10);

    let mut count = 0;
    while count < num {
        if let Ok((size, ins)) = decode_instruction(&rom[*start as usize..]) {
            println!("{:x}  {}", *start, ins);
            *start += size as u16;
            count += 1;
        }
        else {
            println!("Hit invalid instruction at {:04x}", *start);
            break;
        }
    }
